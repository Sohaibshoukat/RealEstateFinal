...
const PaymentPlan = (props) => {
  ...
  return (
    <>
      ...
      <div className={`${style.Plan_head}`}>
        <button className="btn btn-thm" value={"All"} onClick={() => { setPlanSet("All") }}>ALL</button>
        {props.property?.paymentPlan?.map((item) => {
          return (
            <button className="btn btn-thm" value={item.Title} onClick={() => { setPlanSet(item.Title) }}>{item.Title}</button>
          )
        })}
      </div>
            ...
            item?.InPlan?.map((item2) =>
              <div className={style.InPlan}>
                <div className={style.Plan_Over} onClick={() => { setDetailVisble(!DetailVisble) }}>
                  {item2?.Plan && Object.keys(item2?.Plan).map((key, i) =>
                    <p>{item2?.Plan[key]}</p>
                  )}
                </div>
                {DetailVisble &&
